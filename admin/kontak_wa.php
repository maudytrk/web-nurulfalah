<?php
/**
 * Halaman Manajemen Kontak WhatsApp Panitia PPDB per Unit
 * YPI Nurul Falah
 */
...
// Ambil data kontak WA (Filter jika Admin Unit)
...
try {
    if ($unit_filter !== 'all' && in_array($unit_filter, ['ra', 'mi', 'smpi'])) {
        $stmt = $pdo->prepare("SELECT * FROM kontak_wa WHERE target_unit = :unit ORDER BY id ASC");
        $stmt->execute([':unit' => strtoupper($unit_filter)]);
    } else {
        $stmt = $pdo->query("SELECT * FROM kontak_wa ORDER BY id ASC");
    }
    $daftar_wa = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Kontak WhatsApp - YPI Nurul Falah</title>
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800 font-weight-bold">Kelola Kontak WhatsApp Panitia PPDB</h1>
                        <button class="btn btn-success btn-icon-split shadow-sm" data-toggle="modal" data-target="#modalTambah">
                            ...
                            <span class="text">Tambah Kontak WA</span>
                        </button>
                    </div>
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3 bg-white">
                            <h6 class="m-0 font-weight-bold text-islamic">Daftar Nomor Kontak Official WhatsApp</h6>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped" id="dataTable" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        <?php $no = 1; foreach ($daftar_wa as $row): ?>
                                            <tr>
                                                <td><?= $no++; ?></td>
                                                <td class="font-weight-bold"><?= htmlspecialchars($row['nama_kontak']); ?></td>
                                                <td>
                                                    <a href="https://wa.me/<?= htmlspecialchars($row['nomor_wa']); ?>" target="_blank" class="text-success font-weight-bold">
                                                        <i class="fab fa-whatsapp mr-1"></i> <?= htmlspecialchars($row['nomor_wa']); ?>
                                                    </a>
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    <button class="btn btn-sm btn-warning text-white btn-edit"
                                                            data-id="<?= $row['id']; ?>"
                                                            data-nama="<?= htmlspecialchars($row['nama_kontak']); ?>"
                                                            data-nomor="<?= htmlspecialchars($row['nomor_wa']); ?>"
    ...
    <div class="modal fade" id="modalTambah" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <form action="kontak_wa_proses.php" method="POST">
                    <?= csrf_field(); ?>
                    <input type="hidden" name="action" value="tambah">
                    <div class="modal-header bg-success text-white">
                        <h5 class="modal-title font-weight-bold"><i class="fab fa-whatsapp mr-2"></i>Tambah Kontak WhatsApp</h5>
                        <button type="button" class="close text-white" data-dismiss="modal"><span>&times;</span></button>
                    </div>
                    <div class="modal-body">
                        <div class="form-group">
                            <label class="font-weight-bold">Nama Kontak / Panitia <span class="text-danger">*</span></label>
                            <input type="text" name="nama_kontak" class="form-control" placeholder="Contoh: Panitia PPDB RA (Ibu Siti)" required>
                        </div>
                        <div class="row">
                            <div class="col-md-6 form-group">
                                <label class="font-weight-bold">Nomor WhatsApp <span class="text-danger">*</span> <small class="text-muted">(Format: 08xx / 628xx)</small></label>
                                <input type="text" name="nomor_wa" class="form-control" placeholder="Contoh: 081234567890" required>
                            </div>
                            <div class="col-md-6 form-group">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-success"><i class="fas fa-save mr-1"></i> Simpan Kontak</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <div class="modal-content">
                <form action="kontak_wa_proses.php" method="POST">
                    ...
                    <div class="modal-header bg-warning text-white">
                        <h5 class="modal-title font-weight-bold"><i class="fas fa-edit mr-2"></i>Edit Kontak WhatsApp</h5>
                        <button type="button" class="close text-white" data-dismiss="modal"><span>&times;</span></button>
                    </div>
                    <div class="modal-body">
                        <div class="form-group">
                            <label class="font-weight-bold">Nama Kontak / Panitia <span class="text-danger">*</span></label>
                            <input type="text" name="nama_kontak" id="edit_nama" class="form-control" required>
                        </div>
                        <div class="row">
                            <div class="col-md-6 form-group">
                                <label class="font-weight-bold">Nomor WhatsApp <span class="text-danger">*</span></label>
                                <input type="text" name="nomor_wa" id="edit_nomor" class="form-control" required>
                            </div>
                            <div class="col-md-6 form-group">
                                <label class="font-weight-bold">Target Unit <span class="text-danger">*</span></label>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="kontak_wa_proses.php" method="POST">
                    ...
                    <div class="modal-body">
                        Apakah Anda yakin ingin menghapus kontak WA <strong id="hapus_nama"></strong>?
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Batal</button>
